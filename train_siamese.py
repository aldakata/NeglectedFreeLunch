...
if __name__ == "__main__":
    ...
    print(f"Loading data from {data_path}, training for {args.epochs} epochs.\nLogging at {args.log}")
    ...
    dataloader_val = DataLoader(dataset_val, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn)
    dataloader_test = DataLoader(dataset_test, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn)

    print(f"Size of the datasets: train: {len(dataset_train)}, val: {len(dataset_val)}, test: {len(dataset_test)}")
    ...
    for epoch in range(args.epochs):
        ...
        losses.append(loss)
        if epoch % 50 == 0:
            print(f"Epoch {epoch}, loss: {loss}, train accuracy: {train_accuracy}, val accuracy: {val_accuracy}")
